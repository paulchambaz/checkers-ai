  ...
  (let ((gen nil))
    (dotimes (i 32)
      (push
        (make-ai
          :dna (make-dna
                 (+ (nth 0 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 1 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 2 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 3 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 4 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 5 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 6 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 7 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 8 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 9 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 10 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 11 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 12 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 13 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 14 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 15 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 16 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 17 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 18 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 19 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 20 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 21 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 22 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 23 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 24 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 25 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 26 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 27 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 28 (ai-dna champion)) (gaussian 2.5))
                 (+ (nth 29 (ai-dna champion)) (gaussian 2.5))
                 )
          :elo (ai-elo champion))
        gen))
    gen))

(defun average-ai (champion filename)
  "Compute the tournament for the average ai"
  (compute-diagonals)
  (let* ((gen (get-gen-from-champion (nth 0 (load-gen champion))))
        (sim-gen (simulate-gen gen))
        (sorted-gen (sort (copy-gen sim-gen) #'< :key #'ai-elo)))
    (save-gen sorted-gen filename)))
      ...
      (loop for line = (read-line stream nil)
            ...
            do (let ((tokens (split-string line ",")))
                 (push (make-ai
                         ...
                         :elo (nth 0 (parse-number (nth 30 tokens)))) gen))))
    gen))
    ...
    (dotimes (j set-number)
      (dolist (ai generation)
        (let* ((opponent (pick-opponent ai generation))
          ...
          ; (format t "finished set player elo after : ~A, opponent elo after : ~a~%" (ai-elo ai) (ai-elo (nth opponent generation)))
          )))
    generation))
    ...
    (dotimes (i 256)
      (format t "turn ~A~%" i)
        (if (equal (state-player state) +white+)
          (setf state (ai-turn state white))
          (setf state (ai-turn state black)))

        ; terminal testing
        (let* ((terminal (terminal-test state (actions state) +white+))
               (utility (terminal-utility-pair-utility terminal)))
          (when (terminal-utility-pair-terminal terminal)
            (cond ((equal utility 1) (return-from match 1.0))
                  ((equal utility -1) (return-from match 0.0))
                  ((equal utility 0) (return-from match 0.5)))))
        (let* ((terminal (terminal-test state (actions state) +black+))
               (utility (terminal-utility-pair-utility terminal)))
          (when (terminal-utility-pair-terminal terminal)
            (cond ((equal utility 1) (return-from match 1.0))
                  ((equal utility -1) (return-from match 0.0))
...
(defun generate-new-generation (orig-gen)
  ...
  (let ((new-gen nil)
    ...
    (push (make-ai :dna (ai-dna champion) :elo 1000.0) new-gen)
  new-gen))
