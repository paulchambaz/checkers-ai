...
(defun iterative-alpha-beta-search (state depth ai)
  (format t "iterative-alpha-beta-search~%")
  ...
  (let* ((result (make-utility-move-pair :utility nil :move nil))
    ...
    (dotimes (n depth)
      (format t "n: ~a~%" (+ n 1))
      (let ((res (max-value state most-negative-fixnum most-positive-fixnum player (+ n 1) ai)))
        (format t "finished minimax~%")
        (when (< (get-internal-run-time) max-time)
          ; (format t "res: ~a~%" res)
          (setf result res)
          (setf max-depth (+ n 1)))))
    (format t "value: ~a depth: ~a~%" (utility-move-pair-utility result) max-depth)
    (utility-move-pair-move result)))

(defun max-value (state alpha beta player depth ai)
  ; (format t "max-value: state: ~a alpha: ~a beta: ~a player: ~a depth: ~a~%" state alpha beta player depth)
  (if (or (equal depth 0))
...
(defun min-value (state alpha beta player depth ai)
  ; (format t "min-value: state: ~a alpha: ~a beta: ~a player: ~a depth: ~a~%" state alpha beta player depth)
  (if (or (equal depth 0))
    (make-utility-move-pair :utility (utility state player ai) :move nil)
    (let ((best (make-utility-move-pair :utility most-positive-fixnum :move nil))
          ; (fr (format t "min-value made best~%"))
          ; (sorted-actions (order-moves (actions state) (to-move state) depth history killer-moves)))
          (sorted-actions (actions state)))
      ; (format t "min-value we have the moves~%")
      (dolist (move sorted-actions)
        (format t "move: ~a~%" move)
        ; (format t "min-value start of the loop~%")
        (let* ((next-state (result move state))
               (test (progn (format t "state: ~a~%" state)
                            (format t "next-state: ~a~%" next-state)))
               (current (if (equal (to-move next-state) player)
                          (max-value next-state alpha beta player (- depth 1) ai)
                          (min-value next-state alpha beta player (- depth 1) ai))))
          ; (format t "min-value out of loop~%")
          (when (< (utility-move-pair-utility current) (utility-move-pair-utility best))
            ; (format t "min-value updating the best~%")
            (setf (utility-move-pair-utility best) (utility-move-pair-utility current))
            (setf (utility-move-pair-move best) move)
            ; (when (>= alpha beta)
            ;   (return best))
...
(defun utility (state player ai)
  ...
  (let ((state-copy-player (copy-state state))
    ...
    (setf (state-player state-copy-player) player)
    (setf (state-player state-copy-ennemy) (switch-player player))
